...
class objfiles_ct : public objfiles_ct_interface
{
 protected:
 ...
 public:
  ...
  libcwd::object_file_ct const* get_object_file() const override
  {
...
bool WST_initialized = false;                   // MT: Set here to false, set to `true' once in `cwbfd::ST_init'.
struct objfile_ct;

class symbol_ct : public symbol_ct_interface
{
 private:
 ...
 public:
  ...
  bool diecu(Dwarf_Die* cu_die_out) const override
  {
  ...
  char const* name() const override { return linkage_name_ ? linkage_name_ : func_name_; }
...
struct symbol_key_greater
{
  ...
  bool operator()(symbol_ct_key const& a, symbol_ct_key const& b) const
  {
...
class objfile_ct : public objfiles_ct
{
 ...
 public:
  ...
  uintptr_t get_lbase() const override { return M_lbase; }

  uintptr_t get_start_addr() const { return M_start_addr; }
  uintptr_t get_end_addr() const { return M_end_addr; }

  symbol_ct_interface const* find_symbol(symbol_ct_key const& search_key) const override;
...
symbol_ct::symbol_ct(Dwarf_Addr start, Dwarf_Addr end,
    objfile_ct* objfile, Dwarf_Die const* die, char const* cu_name, char const* func_name, char const* linkage_name) :
    symbol_ct_interface(start + objfile->get_lbase(), end + objfile->get_lbase()),
...
symbol_ct_interface const* objfile_ct::find_symbol(symbol_ct_key const& search_key) const
{
...
char const* pc_mangled_function_name(void const* addr)
{
  ...
  symbol_ct_key search_key(reinterpret_cast<uintptr_t>(addr));

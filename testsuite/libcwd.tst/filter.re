...
MALLOC  : Allocated memory: [0-9]* bytes in [2|3] blocks:
          0x[a-f0-9]* tst_filter_shared:libcwd\.tst/filter\.cc:50   std::vector<int, std::allocator<int> >; \(sz = 12\)  filter\.cc
MALLOC  : Allocated memory: [0-9]* bytes in [2|3] blocks:
          0x[a-f0-9]* tst_filter_shared:libcwd\.tst/filter\.cc:50   std::vector<int, std::allocator<int> >; \(sz = 12\)  filter\.cc
MALLOC  : delete 0x[a-f0-9]* <pre libcwd initialization> <unknown type>; \(sz = 4096\)  
...
MALLOC  : malloc\(52\) = 0x[a-f0-9]*
MALLOC  : calloc\(5, 16\) = 0x[a-f0-9]*
MALLOC  : malloc\(68\) = 0x[a-f0-9]*
...
MALLOC  : Removing libcw::debug::marker_ct at 0x[a-f0-9]* \(marker1\)
  ...
  \* realloc   0x[a-f0-9]* module\.so:           module\.cc:31   void\*; \(sz = 1000\)  realloc1000_with_AllocTag
  \* new\[\]     0x[a-f0-9]* module\.so:           module\.cc:38   char\[1000\]; \(sz = 1000\)  new1000
  \* malloc    0x[a-f0-9]* tst_filter_shared:libcwd\.tst/filter\.cc:130  void\*; \(sz = 123\)  Allocated between the two markers
...
MALLOC  : free\(0x[a-f0-9]*\)         dl-version\.c:289  <unknown type>; \(sz = 80\)  
...
MALLOC  : free\(0x[a-f0-9]*\)            dl-deps\.c:528  <unknown type>; \(sz = 52\)  
MALLOC  : Allocated memory: [0-9]* bytes in [5|6] blocks:
[0-9:.]* \(deleted\) 0x[a-f0-9]* tst_filter_shared:           filter\.cc:128  <marker>; \(sz = 4\)  marker1
    [0-9:.]* realloc   0x[a-f0-9]* module\.so:           module\.cc:31   void\*; \(sz = 1000\)  realloc1000_with_AllocTag
    [0-9:.]* malloc    0x[a-f0-9]* tst_filter_shared:           filter\.cc:130  void\*; \(sz = 123\)  Allocated between the two markers

// Verify deterministic resolution of overlapping dwarf2 function symbol ranges.
// The test exercises the private range insertion helper directly so it can cover ELF/DWARF precedence and same-provenance size ordering without needing synthetic object files.
...
bool expect_range(libcwd::dwarf2::FunctionSymbolRanges const& ranges,
    uintptr_t start_addr, uintptr_t end_addr, char const* name, bool is_elf_symbol, uintptr_t symbol_size)
{
  ...
  libcwd::dwarf2::SymbolRange const& symbol = range->second;
...
bool expect_exact_ranges(char const* scenario, libcwd::dwarf2::FunctionSymbolRanges const& ranges, std::vector<ExpectedRange> const& expected)
{
...
void insert_oversized_base_range(libcwd::dwarf2::FunctionSymbolRanges& ranges,
...
void seed_three_base_ranges(libcwd::dwarf2::FunctionSymbolRanges& ranges, bool base_is_elf_symbol, Dwarf_Die const& dwarf_die)
{
...
bool verify_complete_removal_shapes(bool base_is_elf_symbol, bool new_is_elf_symbol)
{
  ...
  {
    libcwd::dwarf2::FunctionSymbolRanges ranges;
    seed_three_base_ranges(ranges, base_is_elf_symbol, dwarf_die);

    Dwarf_Die const* new_die = new_is_elf_symbol ? nullptr : &dwarf_die;
    libcwd::dwarf2::insert_function_symbol_range(ranges, scenario.start_addr, scenario.end_addr, "new_range", new_die);
...
bool verify_dwarf_beats_elf()
{
  ...
  libcwd::dwarf2::insert_function_symbol_range(ranges, 10, 50, "elf_big", nullptr);
  libcwd::dwarf2::insert_function_symbol_range(ranges, 20, 40, "dwarf_inner", &dwarf_die);
...
bool verify_smaller_same_kind_wins()
{
  ...
  libcwd::dwarf2::insert_function_symbol_range(ranges, 100, 200, "dwarf_big", &dwarf_die);
  libcwd::dwarf2::insert_function_symbol_range(ranges, 120, 160, "dwarf_small", &dwarf_die);
...
bool verify_existing_smaller_removes_pending_piece()
{
  ...
  libcwd::dwarf2::insert_function_symbol_range(ranges, 300, 330, "dwarf_small", &dwarf_die);
  libcwd::dwarf2::insert_function_symbol_range(ranges, 290, 350, "dwarf_big", &dwarf_die);
...
bool verify_existing_dwarf_blocks_elf_piece()
{
  ...
  libcwd::dwarf2::insert_function_symbol_range(ranges, 500, 550, "dwarf_middle", &dwarf_die);
  libcwd::dwarf2::insert_function_symbol_range(ranges, 480, 560, "elf_outer", nullptr);

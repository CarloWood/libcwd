...
template<class X, bool array = false>	// Use array == true when `ptr' was allocated with new [].
  ...
  public:
    ...
    lockable_auto_ptr(lockable_auto_ptr const& r) :
        ptr_(r.ptr_), locked_(false), owner_(r.owner_ && !r.locked_)
	{ if (!r.locked_) r.owner_ = 0; }
    ...
    ~lockable_auto_ptr() { if (owner_) { if (array) delete [] ptr_; else delete ptr_; } }
    ...
    bool strict_owner() const { LIBCWD_ASSERT(is_owner()); return locked_; }
    ...
    X* release() const { LIBCWD_ASSERT(is_owner()); owner_ = 0; return ptr_; }
    ...
    void lock() { LIBCWD_ASSERT(is_owner()); locked_ = true; }
      // Lock the ownership.
      // You should only call this when this object is the owner.

...
#define FATALDEBUGDEBUG_CERR(...)                                                                \
  do                                                                                             \
  {                                                                                              \
    std::lock_guard<std::mutex> lock(::libcwd::_private_::raw_write_mutex);                      \
    std::cerr << "CWDEBUG_DEBUG: " << std::this_thread::get_id() << ": " << __VA_ARGS__ << '\n'; \
  } while (0)

#define LIBCWD_DEBUG_ASSERT(...)                                                                               \
  do                                                                                                           \
  {                                                                                                            \
    if (!(__VA_ARGS__))                                                                                        \
    {                                                                                                          \
...
#define DEBUGDEBUG_CERR(...) FATALDEBUGDEBUG_CERR(__VA_ARGS__)
#else // !CWDEBUG_DEBUGOUTPUT
#define DEBUGDEBUG_CERR(...) \
  do                         \
  {                          \
  } while (0)

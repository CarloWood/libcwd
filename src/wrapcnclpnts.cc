// SPDX-FileCopyrightText: 2002-2004, 2018-2019, 2023, 2026 Carlo Wood
...
CANCELLATION_POINT_SYSCALL(int, nanosleep, (const struct timespec* requested_time, struct timespec* remaining),
                           (requested_time, remaining))

CANCELLATION_POINT_SYSCALL_VA(int, open, (char const* pathname, int flags, ...), (pathname, flags, va_arg(ap, mode_t)),
                              flags)
// strong_alias(open, __open)

CANCELLATION_POINT_SYSCALL_VA(int, open64, (char const* pathname, int flags, ...),
                              (pathname, flags, va_arg(ap, mode_t)), flags)
// strong_alias(open64, __open64)

CANCELLATION_POINT_SYSCALL_DLSYM(int, pause, (), ())

CANCELLATION_POINT_SYSCALL_DLSYM(ssize_t, pread, (int fd, void* buf, size_t count, off_t offset),
                                 (fd, buf, count, offset))

CANCELLATION_POINT_SYSCALL(ssize_t, pread64, (int fd, void* buf, size_t count, off64_t offset),
                           (fd, buf, count, offset))
// strong_alias(pread64, __pread64)

CANCELLATION_POINT_SYSCALL_DLSYM(ssize_t, pwrite, (int fd, void const* buf, size_t n, off_t offset),
                                 (fd, buf, n, offset))

CANCELLATION_POINT_SYSCALL(ssize_t, pwrite64, (int fd, void const* buf, size_t n, off64_t offset), (fd, buf, n, offset))
// strong_alias(pwrite64, __pwrite64)

CANCELLATION_POINT_SYSCALL(ssize_t, read, (int fd, void* buf, size_t count), (fd, buf, count))
// strong_alias(read, __read)

CANCELLATION_POINT_SYSCALL_DLSYM(int, system, (char const* line), (line))
...
CANCELLATION_POINT_SYSCALL(ssize_t, write, (int fd, void const* buf, size_t n), (fd, buf, n))
...
CANCELLATION_POINT_SYSCALL_DLSYM(ssize_t, recvfrom,
                                 (int fd, __ptr_t buf, size_t n, int flags, __SOCKADDR_ARG addr, socklen_t* addr_len),

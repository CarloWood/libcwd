...
// a fork handler.
//
int ST_exec_prog(char const* prog_name, char const* const argv[], char const* const envp[],
                 int (*decode_stdout)(char const*, size_t))
{
  Debug(if (!dc::debug.is_on()) libcw_do.off(););
  Dout(dc::debug | continued_cf, "exec_prog(\"" << prog_name << "\", " << cwprint(argv_ct(argv)) << ", "
                                                << cwprint(environment_ct(envp)) << ", decode) = ");
  ...
  switch (ret)
  {
    case 0: {
      ...
      class ofdstream : public std::ostream
      {
       ...
       public:
        explicit ofdstream(int fd) : std::ostream(&M_fdbuf), M_fdbuf(fd) { }
        ...
        for (int fd = 0; fd < number_of_fds; ++fd)
        {
          if ((ufds[fd].revents & POLLIN) == POLLIN)
          {
            ...
            do
            {
              ...
              if (ufds[fd].fd != debug_filedes[0])
#endif
                Dout(dc::finish | cond_error_cf(len == -1),
                     '"' << buf2str(readbuf, len > 0 ? len : 0) << "\", " << sizeof(readbuf) << ") = " << len);
#endif
              if (len <= 0)
              {
              ...
              for (char const* p = readbuf; p < &readbuf[len]; ++p)
              {
                ...
                if (*p == '\n')
                {
                  ...
                  if (ufds[fd].fd == stdout_filedes[0])
                  {
                    ...
                    Dout(dc::warning, "child process returns on stderr: \""
                                          << buf2str(decodebuf[fd].data(), decodebuf[fd].size()) << '"');
#ifdef CWDEBUG
  ...
  Dout(dc::finish, ret);
  Debug(if (!dc::debug.is_on()) libcw_do.on(););
  return ret;
}
